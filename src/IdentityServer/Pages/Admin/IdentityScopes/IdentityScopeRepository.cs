...
public class IdentityScopeRepository
{
    ...
    {
        IQueryable<IdentityResource> query = _context.IdentityResources
        ...
        IQueryable<IdentityScopeSummaryModel> result = query.Select(x => new IdentityScopeSummaryModel
        {
            Name = x.Name,
    ...
    {
        IdentityResource? scope = await _context.IdentityResources
            .Include(x => x.UserClaims)
    ...
    public async Task CreateAsync(IdentityScopeModel model)
    {
        ...
        IEnumerable<string> claims = model.UserClaims?.Split(' ', StringSplitOptions.RemoveEmptyEntries).ToArray() ?? Enumerable.Empty<string>();
        if (claims.Any())
        {
...
#pragma warning restore CA1849
    ...
    public async Task UpdateAsync(IdentityScopeModel model)
    {
        ...
        IEnumerable<string> claims = model.UserClaims?.Split(' ', StringSplitOptions.RemoveEmptyEntries).ToArray() ?? Array.Empty<string>();
    ...
    {
        IdentityResource? scope = await _context.IdentityResources.SingleOrDefaultAsync(x => x.Name == id);
