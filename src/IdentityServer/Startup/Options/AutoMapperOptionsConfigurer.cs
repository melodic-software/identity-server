...
internal sealed class AutoMapperOptionsConfigurer : IConfigureAppOptions
{
    public static void Configure(AppOptions options, IConfiguration configuration, IHostEnvironment environment)
    {
        options.ConfigureAutoMapper(autoMapperOptions =>
        {
            autoMapperOptions.AddAssembly(AssemblyReference.Assembly);
        });
    }
}
